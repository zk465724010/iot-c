...
int CThread11::Run(USER_CBK pCallback, void *pUserData/*=NULL*/, int nUserDataSize/*=0*/)
{
	...
	#endif
	else LOG("WARN", "Thread is already running.\n\n");
	return nRet;
}
...
void CThread11::Join()
{
	...
	else LOG("WARN", "thread join failed, joinable()=%s,joinable=%s, cannot join.\n\n", m_thread.joinable()?"true":"false", m_bJoinable ? "true" : "false");
}
...
void CThread11::Detach()
{
	...
	else LOG("WARN", "thread detach failed, joinable()=%s,joinable=%s, cannot detach.\n\n", m_thread.joinable() ? "true" : "false", m_bJoinable ? "true" : "false");
}
