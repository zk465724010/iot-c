...
void dump_tds__GetCapabilitiesResponse(struct _tds__GetCapabilitiesResponse *rep)
{
    ...
    if (NULL != rep->Capabilities) {
        ...
        if (NULL != rep->Capabilities->Device) {
			...
			log_level_val(2, "XAddr: ", log_str, rep->Capabilities->Device->Network);
			log_level_val(2, "XAddr: ", log_str, rep->Capabilities->Device->System);
			log_level_val(2, "XAddr: ", log_str, rep->Capabilities->Device->IO);
			log_level_val(2, "XAddr: ", log_str, rep->Capabilities->Device->Security);
			log_level_val(2, "XAddr: ", log_str, rep->Capabilities->Device->Extension);
		}
        ...
        if (NULL != rep->Capabilities->Media) {
			...
			log_level_val(2, "XAddr: ", log_str, rep->Capabilities->Media->StreamingCapabilities);
			log_level_val(2, "XAddr: ", log_str, rep->Capabilities->Media->Extension);
		}
